...
\begin{enumerate}
    ...
    \item \textbf{PX4 Converter Node} (\texttt{px4\_converter\_gazebo} for SITL, \texttt{px4\_converter\_node} for flight): A C++ node that subscribes to \texttt{/yaw\_command}, constructs PX4 \texttt{TrajectorySetpoint} messages with the commanded yaw angle and a fixed hover position, and streams them at 10~Hz to maintain offboard mode. The node handles the PX4 arming sequence (offboard mode request followed by arm command) and implements a safety timeout that ceases control output after 5~s without a yaw command.
\end{enumerate}
